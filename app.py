...
DEFAULT_REGION = "IND"  # ✅ if region param missing -> IND
...
def get_name_region_from_reward(access_token):
    try:
        ...
        print("inspect_token status:", uid_res.status_code)
        print("content-type:", uid_res.headers.get("content-type"))
        print("body head:", uid_res.text[:200])
...
def combined_bio_upload():
    ...
    region_in = request.args.get("region") or request.form.get("region")  # optional

    if not bio:
        return jsonify({"status": "❌ Error", "code": 400, "error": "Missing 'bio' parameter"}), 400

    # ✅ Define defaults first (so UnboundLocal can NEVER happen)
    ...
    # Debug (temporary)
    # print("DEBUG URLs:", selected_region, update_url, major_url)
